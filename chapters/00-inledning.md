...
Referensimplementationen verifieras dessutom automatiskt i GitHub Actions. CI kör frontendens komponenttester, backendens API-/integrationstester mot PostgreSQL Dev Services, bygger de separata delarna, skapar Docker-images, startar Compose-miljön och provar ett riktigt API-flöde genom den publika Nginx-ingången. Därmed kontrolleras både komponentbeteende, backendens verkliga persistenskedja och att den sammansatta kedjan Nginx → Quarkus → PostgreSQL fungerar i praktiken. De exakta testnivåerna och vad smoke-testet bevisar behandlas senare i boken.
