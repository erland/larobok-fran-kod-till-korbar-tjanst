...
| F006 | 12 | Aktuell CI-/testsetup, Quarkus HTTP-testning, PostgreSQL Dev Services samt rekommenderad frontend-teststack | Kontrollerad | Faktisk `04-test-reference-implementation.yml`, `pom.xml` och `package.json`; Quarkus *Testing Your Application* och *Dev Services for Databases*; Vitest *Getting Started*; Testing Library *React Testing Library* | 2026-08-16 | Kapitlet skiljer uttryckligen mellan nuvarande verifiering och föreslagna framtida tester: inga backendtestklasser eller frontendtestberoenden är ännu incheckade. |
